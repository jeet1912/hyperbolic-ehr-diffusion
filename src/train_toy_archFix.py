...
def sample_trajectories(
    ...
    for i in range(num_samples):
        ...
        for j in range(max_len):
            codes = sorted(set(int(c) for c in decoded_idx[i, j]))
            traj.append(codes)
...
def main(
    ...
    train_dl = DataLoader(TrajDataset(train_trajs, max_len, pad_idx), batch_size=batch_size,
                          shuffle=True, collate_fn=collate)
    val_dl = DataLoader(TrajDataset(val_trajs, max_len, pad_idx), batch_size=batch_size,
                        shuffle=False, collate_fn=collate)
    test_dl = DataLoader(TrajDataset(test_trajs, max_len, pad_idx), batch_size=batch_size,
                         shuffle=False, collate_fn=collate)
...
if __name__ == "__main__":
    ...
    experiments = [
        ("depth2_final", ToyICDHierarchy(extra_depth=0)),
